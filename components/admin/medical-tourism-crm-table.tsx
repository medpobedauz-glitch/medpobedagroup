...
type MedicalTourismCrmTableProps = {
  items: Awaited<
    ReturnType<typeof import("@/lib/data/dashboard").getMedicalTourismCrmData>
  >["items"];
  staff: Array<{ id: string; name: string }>;
  hospitals: Array<{ id: string; name: string }>;
};

export function MedicalTourismCrmTable({
  items,
  staff,
  hospitals,
}: MedicalTourismCrmTableProps) {
  if (!items.length) {
    return (
      <Card className="border-white/10 p-6 text-slate-300">
        No medical tourism cases match the current filters.
      </Card>
    );
  }
          ...
          <div className="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
              ...
              <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm text-slate-300">
                    <span>{item.patient?.fullName || item.name}</span>
                  ...
                  <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm text-slate-400">
                    <span>Budget: {item.budgetRange || "Not specified"}</span>
                    <span>
                      Hospital: {item.assignedHospital?.name || item.preferredHospital || "Not assigned"}
                    </span>
                    <span>Files: {item.uploadedFiles.length}</span>
                    ...
                    timeline: [
                      {
                        id: "created",
                        label: "Case created",
                        value: "Medical tourism case captured in the CRM.",
                        createdAt: item.createdAt.toISOString(),
                      },
                      ...item.notes.map((note) => ({
                        id: note.id,
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2">
                  {item.uploadedFiles.map((file) => (
                    <Link
                      ...
                      className="rounded-[1rem] border border-white/8 bg-white/6 px-4 py-3 text-sm text-slate-200 transition hover:bg-white/10"
                    >
                      {file.originalName}
                    </Link>
                  ))}
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <input type="hidden" name="model" value="medicalTourismInquiry" />
                ...
                <SubmitButton
                  type="submit"
                  variant="secondary"
                  pendingLabel="Updating case..."
                >
                  Save Case Workflow
                </SubmitButton>
              </form>
              <div className="rounded-[1.5rem] border border-white/10 bg-slate-950/26 p-4">
                <p className="text-sm font-semibold text-white">Follow-up Notes</p>
                ...
                <form action={addLeadNoteAction} className="mt-4 grid gap-3">
                  ...
                  <Textarea
                    name="content"
                    placeholder="Add a follow-up note for this patient case"
