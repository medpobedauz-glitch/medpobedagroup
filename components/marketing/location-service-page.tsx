...
export function createLocationPageMetadata(pageId: LocationPageId) {
...
export function LocationServicePage({ pageId }: { pageId: LocationPageId }) {
  const locale = getRequestLocale();
  const messages = getMessages(locale);
  const config = locationPageConfigs[pageId];
  const common = messages.pages.locationPages.common;
  const page = messages.pages.locationPages[pageId];
  ...
  return (
    <>
      ...
      <LocationLinksSection
        eyebrow={common.relatedLocationsEyebrow}
        title={common.relatedLocationsTitle}
        description={common.relatedLocationsDescription}
        pageIds={config.relatedLocations}
        columns={3}
      />

      <AuthorityLinksSection
        eyebrow={common.relatedAuthoritiesEyebrow}
        title={common.relatedAuthoritiesTitle}
        description={common.relatedAuthoritiesDescription}
