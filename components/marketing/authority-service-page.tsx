...
import {
  authorityPageConfigs,
  type AuthorityPageId,
} from "@/lib/authority-pages";
import { treatmentPageIdsForIndiaPage } from "@/lib/treatment-pages";
import {
...
export function createAuthorityPageMetadata(pageId: AuthorityPageId) {
...
export function AuthorityServicePage({ pageId }: { pageId: AuthorityPageId }) {
  const locale = getRequestLocale();
  const messages = getMessages(locale);
  const config = authorityPageConfigs[pageId];
  const common = messages.pages.authorityPages.common;
  const page = messages.pages.authorityPages[pageId];
  ...
  const ctaSecondaryLabel =
    config.secondaryCtaVariant === "partnerships"
      ? messages.chrome.actions.discussHospitalPartnership
      : messages.ctas.contactOurTeam;
  ...
  return (
    <>
      ...
      {pageId === "treatmentInIndia" ? (
        <TreatmentLinksSection
          eyebrow={messages.pages.treatments.links.eyebrow}
          title={messages.pages.treatments.links.title}
          description={messages.pages.treatments.links.description}
          pageIds={treatmentPageIdsForIndiaPage}
          columns={3}
        />
      ) : null}
